<template>
  <div class="animated fadeIn">
    ...
    <b-modal id="modalInfo" centered @hide="resetModal" title="Hello" v-if="draftPost" hide-footer footerBgVariant="info">
      <b-row>
        <b-col md="12">
          <h5>{{ draftPost.type }}</h5>
          <p>{{ draftPost.comments }}</p>
          <div slot="modal-footer" class="w-100">
            <p class="float-left">Updated Date {{ draftPost.updatedAt}}</p>
            <b-dropdown id="actions" text="Edit" class="m-md-2 float-right">
              ...
              <b-dropdown-item @click.prevent="destroySchedulePost(draftPost.id)">Delete</b-dropdown-item>
            </b-dropdown>
          </div>
        </b-col>
      </b-row>
...
<script>
  ...
  export default {
    ...
    data () {
      return {
        fields: [
          ...
          { key: 'updatedAt', label: 'Update At', sortable: true },
    ...
    methods: {
       ...
       */
      setPage (page) {
        this.$store.dispatch('draftpost/all', (proxy) => {
       ...
       */
      reload () {
        this.$store.dispatch('draftpost/all')
      },
      /**
       * Delete the resource
       */
      destroySchedulePost (staff) {
        this.$store.dispatch('draftpost/destroy', staff)
