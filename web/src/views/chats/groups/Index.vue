          ...
          <b-collapse id="collapse1" visible>
            <b-card-body>
              ...
              <b-table
                ...
                :per-page="group.pagination.perPage"
                :sort-by.sync="sortBy"
              ...
              <b-row>
                <b-col md="6" class="my-1">
                  <b-pagination
                    :total-rows="group.pagination.totalCount"
                    :per-page="limit"
                    v-model="currentPage"
                    prev-text="Prev"
                    next-text="Next"
...
<script>
  ...
  export default {
    ...
    computed: {
      ...
      limit: {
        get () {
          return this.group.pagination.limit
        },
        set (limit) {
          this.setLimit(limit)
        }
      ...
      currentPage: {
        get () {
          return this.group.pagination.currentPage
        },
        set (page) {
          this.setPage(page)
        }
    ...
    methods: {
      onFiltered (filteredItems) {
        ...
        this.currentPage = this.group.pagination.currentPage
      },
      /**
       * Method used to get the group route.
       *
       * @param {Number} id The group identifier.
       *
       * @returns {Object} The group route.
       */
      getGroupRoute (id) {
        return {
          name: 'groups.show',
      ...
      /**
       * Method used to redirect the group to the group create page.
       */
       ...
       */
      setPage (page) {
        this.$store.dispatch('group/all', (proxy) => {
      ...
      destroyGroup (staff, index) {
        this.$store.dispatch('group/destroy', staff)
    ...
    mounted () {
      this.$store.watch((state) => {
        if (state.auth.authenticated) {
          ...
          // this.$echo
          //   .channel('group')
          //   .listen('User.Created', staff => this.$store.dispatch('group/created', staff))
          //   .listen('User.Updated', (staff) => {
          //     console.log(staff)
          //     this.$store.dispatch('group/updated', staff)
          //   })
          //   .listen('User.Deleted', staff => this.$store.dispatch('group/destroyed', staff))
        }
      })
    }
  }
